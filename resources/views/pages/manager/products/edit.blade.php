            ...
            <form wire:submit="save" class="space-y-6">
                ...
                <div>
                    <flux:field label="Product Photo">
                        <input type="file" id="new-photo-input" wire:model="new_photo" accept="image/*"
                            capture="environment"
                        ...
                        <div class="mt-3">
                            @if($new_photo instanceof \Livewire\Features\SupportFileUploads\TemporaryUploadedFile)
                                ...
                                <flux:text class="text-xs text-neutral-600 dark:text-neutral-400 mt-1">New photo preview
                                </flux:text>
                            @elseif($photo)
                                <div class="flex items-center gap-4">
                                    <img src="{{ Storage::disk('public')->url($photo) }}" alt="Current"
                                        class="h-24 w-24 rounded object-cover" />
                                    <flux:button type="button" variant="outline" size="sm" wire:click="removePhoto"
                                ...
                                <flux:text class="text-xs text-neutral-600 dark:text-neutral-400 mt-1">Current photo
                                </flux:text>
                            @else
                                <flux:text class="text-sm text-neutral-600 dark:text-neutral-400">No photo uploaded
                                </flux:text>
                            @endif
                        </div>
                    </flux:field>
                ...
                <!-- Barcode -->
                <flux:input wire:model="barcode" label="Barcode" placeholder="Leave empty for auto-generation" />
                @error('barcode')
                    <flux:text class="text-sm text-red-600 dark:text-red-400">{{ $message }}</flux:text>
                @enderror
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid gap-4 md:grid-cols-2">
                        <flux:input wire:model="price_per_unit" label="Sale Price per {{ $current_unit }}" type="number"
                            step="0.01" min="0" placeholder="0.00" required />
                        @error('price_per_unit')
                            <flux:text class="text-sm text-red-600 dark:text-red-400">{{ $message }}</flux:text>
                        @enderror
                        <flux:input wire:model="purchase_price" label="Purchase Price per {{ $current_unit }}" type="number"
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid gap-4 md:grid-cols-2">
                        <!-- Stock Quantity -->
                        <flux:input wire:model="stock_quantity" label="Stock Quantity" type="number" step="0.01" min="0"
                ...
                <!-- Product Stats -->
                <div
                    class="rounded-none border border-neutral-200 bg-neutral-50 p-4 dark:border-neutral-700 dark:bg-neutral-800">
                    ...
                    <div class="grid gap-4 md:grid-cols-3">
                        <div class="text-center">
                            ...
                            <flux:text class="text-lg font-semibold">{{ number_format($product->total_sold, 2) }}
                                {{ $product->unit_type }}
                            </flux:text>
                        </div>
                        <div class="text-center">
                            ...
                            <flux:text class="text-lg font-semibold text-green-600 dark:text-green-400">
                                ₦{{ number_format($product->total_revenue, 2) }}</flux:text>
                        </div>
                        <div class="text-center">
                ...
                <div class="flex flex-col sm:flex-row items-center justify-end gap-4 pt-6">
                    <flux:button variant="outline" type="button" :href="route('manager.products.index')" wire:navigate
